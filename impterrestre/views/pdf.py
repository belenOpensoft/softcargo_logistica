...
from impterrestre.models import (VEmbarqueaereo, ImpterraEmbarqueaereo, ImpterraCargaaerea, ImpterraConexaerea)
...
def get_datos_caratula(request):
    ...
    if is_ajax(request):
        try:
            ...
            ruta = ImpterraConexaerea.objects.filter(numero=id).order_by('-id').values_list('salida', 'llegada').first()
            ...
            embarque_items = ImpterraCargaaerea.objects.filter(numero=id)
